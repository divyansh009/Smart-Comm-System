    ...
    # read the image file and extract its pixels
        ...
        # cv2.imshow("Output", frame)
            if len(landmarks)==42:
                ...
                refslope1.append(slopee[1])
                dist01.append(distratiot[0])
                dist21.append(distratiot[2])
                dist31.append(distratiot[3])
                dist41.append(distratiot[4])
                slope01.append(sloperatiot[0])
                slope21.append(sloperatiot[2])
                slope31.append(sloperatiot[3])
                slope41.append(sloperatiot[4])
                itr+=1
        if(itr>3000):
            break
    
dic={'slope':refslope,'dist0':dist0,'dist2':dist2,'dist3':dist3,'dist4':dist4,'slope0':slope0,'slope2':slope2,'slope3':slope3,'slope4':slope4,'slopet':refslope1,'dist01':dist01,'dist21':dist21,'dist31':dist31,'dist41':dist41,'slope01':slope01,'slope21':slope21,'slope31':slope31,'slope41':slope41,'label':label}
df=pd.DataFrame.from_dict(dic)
df.to_csv('datasetwords.csv',index=False)
